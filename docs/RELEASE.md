...
8. Create an immutable `v0.5.1` GitHub release only after the above checks pass. Publish SHA-256 values for release assets and verify the tag points to the reviewed commit. Keep v0.5.0 unchanged and disclose its custom-template limitations.
9. Test the README commands from a clean checkout using `--ref v0.5.1`; do not publish a dirty working tree or planning/Vault artifacts.
