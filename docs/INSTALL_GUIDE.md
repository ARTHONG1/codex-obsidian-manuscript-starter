...
현재 허용하는 Python 건너뜀은 최대 세 건입니다. 실 wheelhouse가 제공되지 않은 `test_real_wheelhouse_recreates_committed_lock_when_provided`, 현재 권한에서 디렉터리 reparse point를 만들 수 없는 `test_rejects_reparse_point_without_following_it_when_supported`, `test_snapshot_rejects_reparse_staging_parent_when_supported`이며, 다른 수가 보고되면 검증은 실패해야 합니다.

v0.7.0 릴리스 후보는 다음 순서로 로컬 검증합니다.
