...
class ReleasePackageTests(unittest.TestCase):
    def test_two_builds_have_identical_archive_bytes(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            hashes = []
            for output in roots:
                result = run_powershell(BUILD, '-SourceRoot', str(ROOT), '-OutputRoot', str(output), '-Version', '0.7.1')
                self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
                hashes.append(hashlib.sha256((output / 'codex-obsidian-manuscript-starter-v0.7.1.zip').read_bytes()).hexdigest())
                time.sleep(2.1)  # ZIP timestamps have two-second resolution.
            self.assertEqual(hashes[0], hashes[1])

    def test_fresh_checkout_skill_pair_passes_installer_hash_verification(self):
        # A checkout's EOL conversion must not invalidate the install manifest.
        for autocrlf in ("false", "true"):
            with self.subTest(autocrlf=autocrlf), tempfile.TemporaryDirectory(prefix="skill-checkout-") as temporary:
                snapshot = Path(temporary)
                checkout = subprocess.run(
                    ["git", "-C", str(ROOT), "-c", f"core.autocrlf={autocrlf}",
                     "checkout-index", "--all", f"--prefix={snapshot.as_posix()}/"],
                    capture_output=True, text=True,
                )
                self.assertEqual(checkout.returncode, 0, checkout.stderr)
                module = snapshot / "bootstrap/lib/CodexSkills.psm1"
                manifest = snapshot / "bootstrap/codex-skills-manifest.json"
                quote = lambda value: "'" + str(value).replace("'", "''") + "'"
                command = (
                    f"Import-Module {quote(module)} -Force; "
                    f"$result = Test-CodexSkillSource -ReleaseRoot {quote(snapshot)} "
                    f"-ManifestPath {quote(manifest)}; "
                    "if (-not $result.Valid) { $result.Errors; exit 1 }; "
                    "if ($result.Skills.Count -ne 2) { exit 2 }"
                )
                # PowerShell 7's inherited module path can hide the Windows
                # PowerShell Utility module used by the actual installer.
    ...
    def test_build_creates_exactly_allowlisted_sorted_forward_slash_members_and_checksum(self):
        with tempfile.TemporaryDirectory(prefix="release-package-") as temporary:
            ...
            archive = output / "codex-obsidian-manuscript-starter-v0.7.1.zip"
            ...
            self.assertEqual(release_manifest["version"], "0.7.1")
            self.assertEqual(release_manifest["tag"], "v0.7.1")
            ...
            with ZipFile(archive) as package:
                ...
                self.assertIn("docs/RELEASE_NOTES_v0.7.1.md", names)
    ...
    def test_two_builds_have_identical_member_and_content_identity(self):
        with tempfile.TemporaryDirectory(prefix="release-reproducible-") as temporary:
            ...
            for index in (1, 2):
                ...
                self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
                with ZipFile(output / "codex-obsidian-manuscript-starter-v0.7.1.zip") as package:
                    identities.append(
                        [
