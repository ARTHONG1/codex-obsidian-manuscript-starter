...
class PublicationExportTestCase(BlogPackageMixin, unittest.TestCase):
    ...
    def create_historical_book_bundle(self, version: str = "v0.2", *, archived: bool = False) -> Path:
        """Stored-library fixture, independent of the removed book renderers."""
        item = self.publication_root / "AAA AI Agent Automation" / "01 출판 원고형" / "Part 1" / "기존 책 원고"
        bundle = item / "99 이전버전" / version if archived else item / "00 최신본"
        files = {
            "01 본문-복사용.txt": "기존 본문입니다.".encode("utf-8"),
            "02 원고.md": "# 기존 책 원고".encode("utf-8"),
            "03 미리보기.html": b"<!doctype html><p>Historical book</p>",
            "04 인쇄용.pdf": b"%PDF-1.4\n% historical fixture\n%%EOF\n",
            "05 이미지-삽입순서.md": "# 기존 이미지 순서".encode("utf-8"),
            "images/01-preview.png": b"historical image bytes",
        }
        for relative, content in files.items():
            path = bundle / relative
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(content)
        exported = {relative: hashlib.sha256(content).hexdigest() for relative, content in files.items()}
        fingerprint_input = {
            "profile": "book_a4",
            "project_destination_root": "AAA AI Agent Automation",
            "source_version": version,
            "source_hashes": {"manuscript.json": hashlib.sha256(b"historical source").hexdigest()},
        ...
        manifest = {
            ...
            "source_hashes": fingerprint_input["source_hashes"],
            "validation_status": "ready",
            "vault_publication_status": "published",
        ...
        metadata = bundle / "_meta" / "export-manifest.json"
        metadata.parent.mkdir()
        metadata.write_text(json.dumps(manifest, ensure_ascii=False), encoding="utf-8")
        return bundle
...
class ValidBundleTests(PublicationExportTestCase):
    def test_blog_export_works_without_removed_book_runtime_modules(self):
        source = self.create_rendered_blog()
        import_module = exporter.importlib.import_module

        def without_book_modules(name, *args, **kwargs):
            if name in {"book_v3", "validate_manuscript", "render_manuscript"}:
                raise ModuleNotFoundError(name=name)
            return import_module(name, *args, **kwargs)

        with mock.patch.object(exporter, "validate_blog", None):
            with mock.patch.object(exporter.importlib, "import_module", side_effect=without_book_modules):
                result = exporter.export_publication_bundle(self.request(source))

        self.assertEqual(result["status"], "exported")
        latest = Path(result["latest_path"])
        self.assertIn("[이미지 01 삽입: 대표이미지]", (latest / "01 본문-복사용.txt").read_text(encoding="utf-8"))

    def test_custom_finalizer_exports_markdown_html_and_pdf_without_touching_historical_books(self):
        from finalize_custom_publication import finalize_custom_publication

        historical = self.create_historical_book_bundle()
        before = tree_hashes(historical.parent)
        source = self.sources / "custom" / "v0.1"
        desktop = self.publication_root / "사용자 지정 원고"
        result = finalize_custom_publication({
            "title": "사용자 지정 원고",
            "blocks": [{"component": "paragraphs", "text": "사용자의 문장을 온전히 보존합니다."}],
        }, source, desktop)
    ...
    def test_root_index_is_offline_and_preserves_historical_book_entries(self):
        historical = self.create_historical_book_bundle()
        archived = self.create_historical_book_bundle("v0.1", archived=True)
        before = tree_hashes(historical.parent)
        ...
        self.assertIn("기존 책 원고", page)
        self.assertIn("출판 원고형", page)
        self.assertIn("04%20%EC%9D%B8%EC%87%84%EC%9A%A9.pdf", page)
...
class ValidationAndSecurityTests(PublicationExportTestCase):
    def test_rejected_book_reexport_preserves_existing_library_index_and_files(self):
        historical = self.create_historical_book_bundle()
        self.create_historical_book_bundle("v0.1", archived=True)
        exporter.regenerate_root_index(self.publication_root)
        source = self.sources / "legacy-book" / "v0.3"
        source.mkdir(parents=True)
        (source / "manuscript.json").write_text(json.dumps({
            "output_profile": "book_a4", "template_version": 3,
        }), encoding="utf-8")
        before = tree_hashes(self.root)

        self.assert_error_code("unsupported_profile", self.request(source))

        self.assertEqual(tree_hashes(self.root), before)
        self.assertTrue((historical / "04 인쇄용.pdf").is_file())

    def test_legacy_book_packages_are_rejected_without_changing_source_or_library(self):
        for template_version in (1, 2, 3):
            with self.subTest(template_version=template_version):
                source = self.sources / f"legacy-book-{template_version}" / "v0.1"
                source.mkdir(parents=True)
                (source / "manuscript.json").write_text(json.dumps({
                    "output_profile": "book_a4", "template_version": template_version,
                }), encoding="utf-8")
                before = tree_hashes(self.root)
    ...
    def test_missing_runtime_dependency_returns_json_error_without_traceback(self):
        ...
        self.assertEqual(json.loads(stdout.getvalue())["code"], "python_dependency_missing")

    def test_cli_explicitly_rejects_legacy_book_without_traceback(self):
        source = self.sources / "legacy-book" / "v0.1"
        source.mkdir(parents=True)
        (source / "manuscript.json").write_text('{"output_profile": "book_a4"}', encoding="utf-8")
        before = tree_hashes(source)

        result = subprocess.run([
            str(PYTHON), str(SCRIPTS_ROOT / "export_publication_bundle.py"),
            "--source-version-dir", str(source),
        ...
        self.assertEqual(payload["code"], "unsupported_profile")
        self.assertIn("book_a4", payload["error"])
        self.assertIn("finalize_custom_publication", payload["error"])
    ...
    def test_existing_item_reparse_point_is_rejected_when_supported(self):
        ...
        create_directory_link(profile_link, escaped)
