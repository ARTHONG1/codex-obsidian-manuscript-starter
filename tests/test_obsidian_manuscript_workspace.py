...
CUSTOM_WORKFLOW = PLUGIN_ROOT / "skills" / "obsidian-manuscript-publisher" / "references" / "custom-manuscript-workflow.md"
...
class ObsidianManuscriptWorkspaceTests(unittest.TestCase):
    def test_skill_supports_on_demand_refresh_and_versioned_synthesis(self):
        ...
        for required_text in [
            ...
            "on-demand Codex action",
            "Synthesize a Custom Manuscript Version",
            "references/custom-manuscript-workflow.md",
            "v0.N",
            "custom_manuscript",
        ]:
            self.assertIn(required_text, skill)

    def test_skill_keeps_visual_review_policy_for_supported_profiles(self):
        skill = SKILL.read_text(encoding="utf-8")
        for required_text in [
    ...
    def test_skill_selects_independent_custom_and_blog_output_profiles(self):
        skill = SKILL.read_text(encoding="utf-8")
        for required_text in [
    ...
    def test_default_prompt_routes_only_the_action_requested_by_the_user(self):
        ...
        self.assertIn("블로그", prompt)
        self.assertRegex(prompt, "맞춤|템플릿|사용자 양식")
        self.assertNotIn("A4", prompt)
    ...
    def test_publication_contract_documents_exact_preflight_and_non_destructive_failure(self):
        skill = SKILL.read_text(encoding="utf-8")
    ...
    def test_skill_exports_only_after_ready_validation_and_render(self):
        ...
        for required_text in [
            ...
            self.assertIn(required_text, skill)
        for script in ("render_custom_manuscript.py", "render_blog.py", "export_publication_bundle.py"):
    ...
    def test_publication_library_reference_defines_copy_ready_bundle_and_cli(self):
        ...
        for required_text in [
            ...
            "03 미리보기.html",
            "04 이미지-삽입순서.md",
            "finalize_custom_publication.py",
            "manuscript.md",
            "manuscript.html",
            "manuscript.pdf",
    ...
    def test_custom_and_blog_references_define_separate_publication_contracts(self):
        custom_workflow = CUSTOM_WORKFLOW.read_text(encoding="utf-8")
        ...
        for required_text in [
            ...
            self.assertIn(required_text, custom_workflow)
        for required_text in [
