...
def inspect_verified_package(request: ExportRequest) -> VerifiedPackage:
    ...
    has_manuscript = (source / "manuscript.json").is_file()
    has_blog = (source / "blog.json").is_file()
    if has_manuscript == has_blog:
        raise ExportError("profile_ambiguous", "exactly one profile metadata file is required")
    if has_manuscript:
        raise ExportError(
            "unsupported_profile",
            "built-in book_a4 exports were removed; use adaptive_blog or "
            "finalize_custom_publication for custom_manuscript exports",
        )
    return _inspect_blog(request, source, project)
...
def build_bundle(package: VerifiedPackage, staging_dir: Path) -> dict:
    ...
    markdown_name = "02 블로그.md"
    guide_name = "04 이미지-삽입순서.md"
    copy_text = _blog_copy_text(package)
...
def _usage_content() -> str:
    return (
        ...
        "3. 03 미리보기.html은 화면 확인에 사용합니다. 기존 보관 원고의 PDF도 유지됩니다.\n"
...
def regenerate_root_index(publication_root: Path) -> Path:
    ...
    for manifest_path in root.glob("**/00 최신본/_meta/export-manifest.json"):
        ...
        pdf_link = ""
        # Read-only compatibility for historical book bundles; no new book export.
        if profile == "book_a4":
