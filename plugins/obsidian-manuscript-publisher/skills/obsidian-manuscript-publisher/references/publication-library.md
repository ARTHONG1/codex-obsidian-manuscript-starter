...
- `원고로 완성해줘`: select an approved user template and follow `custom-manuscript-workflow.md`; its dedicated custom finalizer reports the output folder.
...
For supported profiles, use this order:
...
Blog publication uses `publish_manuscript_version.py` and `export_publication_bundle.py`. Custom output uses `finalize_custom_publication.py`. The generic blog exporter does not accept a custom `manuscript.json`.
...
3. Open `04 이미지-삽입순서.md` for a blog. Upload the matching numbered file from `images` at each marker.
4. Use `03 미리보기.html` to check order and appearance.
...
The existing custom finalizer renders `manuscript.md`, `manuscript.html`, and `manuscript.pdf` to the explicitly selected version folder and may export those files to the requested desktop destination. It does not use the blog copy-ready bundle layout shown above. Preserve previously exported book folders and their existing library entries; do not generate or re-export built-in book versions.
